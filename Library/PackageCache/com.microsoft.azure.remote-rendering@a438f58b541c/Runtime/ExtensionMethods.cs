...
namespace Microsoft.Azure.RemoteRendering
{
    ...
    {
        /// <summary>
        /// Provides a platform-specific default for the <see cref="RendererInitOptions" /> supplied on session connection.
        /// Use this instead of default construction to get optimized options depending on the
        /// device that is running ARR.
        /// </summary>
        /// <remarks>
        /// Enable <see cref="RendererInitOptions.ForceTransparencyWritesDepth"/> by default for all platforms.
        /// VR applications typically render sky, immersive worlds, rooms etc. to facilitate user orientation.
        /// Since transparent objects don't write depth as a default in ARR, these would be immediately obscured
        /// by the locally rendered background.
        /// On AR platforms the scene could be filled with additional local content as well, creating visual issues.
        /// Nonetheless, this also keeps the behavior consistent on multiplatform applications.
        /// </remarks>
        public static RendererInitOptions PlatformDefaults => new RendererInitOptions(ServiceRenderMode.Default, false, true);
    }
}
...
namespace Microsoft.Azure.RemoteRendering.Unity
{
    ...
    public static class CommonExtensions
    {
        ...
        public static UnityEngine.Plane toUnity(this Plane plane)
        {
            return new UnityEngine.Plane(new Vector3(plane.A, plane.B, plane.C), -plane.D);
        }
        ...
        public static UnityEngine.Bounds toUnity(this Bounds bounds)
        {
            if(!bounds.IsValid())
            {
                return new UnityEngine.Bounds(new Vector3(0, 0, 0), new Vector3(-1, -1, -1));
            }
        ...
        public static Plane toRemote(this UnityEngine.Plane plane)
        {
            return new Plane(plane.normal.x, plane.normal.y, plane.normal.z, -plane.distance);
        }
        ...
        /// <summary>
        /// Checks that the bounds are valid and can be used for subsequent operations.
        /// Invalid bounds indicate that they don't enclose anything.
        /// </summary>
        public static bool IsValid(this UnityEngine.Bounds bounds)
        {
            return bounds.extents.x >= 0 && bounds.extents.y >= 0 && bounds.extents.z >= 0;
        }
